...
class TestU2SetupAndCoreAdapters:
    ...
    def test_find_all_u2_id_uses_instance_scan(self):
        bot = DamaiBot(config=_u2_config(), setup_driver=False)

        def _selector_for(kwargs):
            sel = Mock()
            sel.exists = Mock(return_value=kwargs.get("instance", 0) < 2)
            return sel

        bot.d = Mock(side_effect=lambda **kwargs: _selector_for(kwargs))
        bot.driver = bot.d
    ...
    def test_press_keycode_safe_u2(self):
        ...
        bot.d.press.assert_called_once_with(66)
